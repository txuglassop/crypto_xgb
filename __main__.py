...
from settings import data_info, feature_settings, train_test_settings
...
def main():
    ...
    eval_metric = train_test_settings['eval_metric']
    session.add_metric(eval_metric.__name__)
    
    params = stepwise_optimisation(X_train, y_train, 5, eval_metric)
    print(params)
    ...
    backtest = Backtest(
        ...
        starting_capital=train_test_settings['starting_capital'], commission=train_test_settings['commission']
    )

    strategy = train_test_settings['strategy']
    session.add_strategy(strategy.__name__)

    bt_results = backtest.run_backtest(strategy, retrain=train_test_settings['retrain'], train_frequency=train_test_settings['train_frequency'])
